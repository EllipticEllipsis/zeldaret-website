...
let mmAssetSeriesConfiguration = [{
	name: 'Asset Reconstruction',
	yAxisVariable: 'assetPercent'
}];

let mmChart = null;
let mmAssetChart = null;
...
window.addEventListener('load', (event) => {
	mmChart = new ProgressChart(
		...
		'mmToggleMatching',
		// Chart Title
		'Majora\'s Mask Code Decompilation Progress History',
		// Chart Subtitle
		'MM USA',
...
`;
	...
	mmAssetChart = new ProgressChart(
		// Chart Element ID
		'mmAssetProgressChart',
		// Matching Toggle Button Element ID
		// 'mmToggleMatching',
		null,
		// Chart Title
		'Majora\'s Mask Asset Reconstruction Progress History',
		// Chart Subtitle
		'MM USA',
		// Non-matching CSV URL
		'https://zelda64.dev/reports/progress_mm.us.rev1.csv',
		...
		function(e) {
			...
			e.series.forEach(function(series, seriesIndex) {
				...
				rows.forEach(function(row, rowIndex) {
					...
					row.x = row.timestamp;
					row.y = row[mmAssetSeriesConfiguration[seriesIndex].yAxisVariable];
					// TODO: How to read this from the chart itself?
				});
			});
			...
			if (typeof lastRow !== 'undefined') {
				...
				document.getElementById("mmLastChange").innerText = date.toLocaleDateString() + " " + date.toLocaleTimeString();
				// document.getElementById("mmTotalPercentFileName").innerText = formatPercent(lastRow.totalPercent);
				document.getElementById("mmAssetsTotalPercent").innerText = formatPercent(lastRow.assetPercent);
				document.getElementById("mmAssetsAudioPercent").innerText = formatPercent(lastRow.audioPercent);
				document.getElementById("mmAssetsMiscPercent").innerText = formatPercent(lastRow.miscPercent);
				document.getElementById("mmAssetsObjectPercent").innerText = formatPercent(lastRow.objectPercent);
				document.getElementById("mmAssetsScenePercent").innerText = formatPercent(lastRow.scenePercent);
				document.getElementById("mmAssetsTexturePercent").innerText = formatPercent(lastRow.texturePercent);
			}
		},
		// Chart Tooltip Formatter Function
		function(tooltip) {
